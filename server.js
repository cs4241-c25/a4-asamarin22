...
// DELETE route - Only authenticated users can delete the last rental entry
server.delete("/delete", ensureAuthenticated, async (req, res) => {
    try {
...
// PUT route - Only authenticated users can modify their own rental entry
server.put("/modify", ensureAuthenticated, async (req, res) => {
    try {
        ...
        // Fetch the rental entry first to verify ownership
        const rentalEntry = await collection.findOne({ "Full Name": fullName });
